// Función para elegir un loot basado en probabilidad
function seleccionarLootAleatorio(loots) {
    const totalProbabilidad = loots.reduce((sum, item) => sum + item.probabilidad, 0);
    let random = Math.random() * totalProbabilidad;

    for (const loot of loots) {
        if (random < loot.probabilidad) return loot;
        random -= loot.probabilidad;
    }
